...
static const char szModule[]="MovieLister.dll";
...
BOOL CALLBACK TitleDesc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		else
		{
			...
			SetDlgItemText(hwnd, IDC_MOVIEDESC, lpLister->m_szItemDesc);
	...
	case WM_COMMAND:
	{
		...
		case IDC_NEXT:
		{
			...
			GETTEXTEX(szTemp, IDC_MOVIEDESC);
			lpLister->SetItemDesc(szTemp);
			SAFE_DELETE_ARRAY(szTemp);
...
BOOL CGameLister::CreateListing(char szFilename[MAX_PATH])
{
	...
	if(m_szItemDesc)
		fprintf(fout, "<font color=black>%s</font>", m_szItemDesc);
	fprintf(fout, "</td></tr>");
...
BOOL CGameLister::SetItemDesc(LPSTR szItemDesc)
{
	COPYSTRINGEX(m_szItemDesc, szItemDesc);
